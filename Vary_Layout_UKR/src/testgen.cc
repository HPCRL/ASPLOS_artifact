...
void testAvx2Scatter(int b, int f, int x, int y, int c, int w, int h, int numA, int strideX=1, int strideY=1){
    ukrgen::Avx2Builder<float> avx2_sp;
...
//    int numA = 6;
    ...
    //In(b,c,x+w, y+s)
    int xwindow_In_stride = (strideY*y+h-1);
...
//    int Ker_c_stride = w*h*f;
    ...
    int In_c_stride = (strideX*x + w-1) * (strideY*y + h-1);
...
//    string code = ukrloop + ukraddup;
    ...
    funcname += to_string(numVB)+"v_cxycgemm";
    ukrfunc+= funcname;
...
//    string genukr_file_name = "gen_ukr_bfxycwh_Ker16f_In1c_Out1f_v"+to_string(numA)+"_";
    ...
    string genukr_file_name = "gen_ukr_A"+to_string(numA)+"B" + to_string(numVB) + "gemm";
...
void testAvx2CnnNoTrans(int b, int f, int x, int y, int c, int w, int h,
                        int num_A, int num_B, string ukrtype, int csplit, int fsplit,
...
//reserved r8, r9, r10, r11, r12,r13
    ...
    int xwindow_In_stride = (strideY*y+h-1)* 16;
    int ywindow_In_stride = 1 * 16;
    int In_c_stride = (strideX*x + w-1) * (strideY*y + h-1) * 16;
    ...
    if(h == 3 && numA <=3 && fsplit == 24){
        ...
        xwindow_In_stride = (strideY*y+h-1)* 8;
        ywindow_In_stride = 1 * 8;
        In_c_stride = (strideX*x + w-1) * (strideY*y + h-1) * 8;
...
//    testAvx2Builder();
        ...
        int fsplit = atoi(argv[11]);
        assert(fsplit == 16 || fsplit == 24);
        int strideX = 1;
        int strideY = 1;
        if(argc >12)strideX=atoi(argv[12]);
        if(argc >12)strideY=atoi(argv[13]);
        // ker_type defines (kf, ic, of)
        // 1: (16,1,1)
        // 2: split c rotate
        // 3: split c gemm
        if(ker_type==1)testAvx2Scatter(b,f,x,y,c,w,h, numA, strideX, strideY);
        else if(ker_type==2) testAvx2CnnNoTrans(b,f,x,y,c,w,h, numA, numB, "rotate", 8, fsplit );
        else if (ker_type==3) testAvx2CnnNoTrans(b,f,x,y,c,w,h, numA, numB, "gemm", 8 , fsplit, strideX, strideY);
        else if (ker_type==4) testAvx2CnnNoTrans(b,f,x,y,c,w,h, numA, numB, "gemm", 16, fsplit, strideX, strideY );
        else{cout<<"no ukr!\n";}
