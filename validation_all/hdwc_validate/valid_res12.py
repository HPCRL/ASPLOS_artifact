...
def run_instance(Txy, Tf, Tc):
    cmd = 'icpc -O3 -Ofast -march=native -fopenmp -I../Vary_Layout_UKR/build -DLKF=16 -DLC=1 -DLOF=1 -DuNf=512 -DuNx=7 -DuNy=7 -DuNc=512 -DuNw=3 -DuNh=3 -DEdgeXY=1 -DuSx=1 -DuSy=1 testbed_likwid.cpp -DLIKWID_PERFMON -Wl,-rpath=/opt/local/lib -llikwid '
...
def gen_pred(Nxy, Nf, Nc, Txy, Tf, Tc, cache_size, bandwidth):
    ...
    # reg,  Rxy, Rf, 1 -> Tc
    Input.append( (1*1+2)*(1*Rxy)*3*Nc  *  R_tiles )
    Kernel.append( Rf*3*3*Nc * R_tiles)
    Output.append( Rxy*Rf * Nc/Tc * R_tiles)
    if Tf == Rf:
        Rxy=Txy
    #L1,  Rxy, Rf, Tc, -> Tf
    Input.append( (1*1+2)*(1*Rxy+2)*Tc *  (Nxy/Rxy*Nf/Tf*Nc/Tc)   )
    ...
    Input.append(  (1*Tyy+2)*(1*Txx+2)*Tc  *  (Nxy/Txy*Nf/Tf*Nc/Tc) )
    Kernel.append( Tf*3*3*Tc  * (Nxy/Txy*Nf/Tf*Nc/Tc))
    Output.append( Txy*Tf  * (Nxy/Txy*Nf/Tf*Nc/Tc))
    if (Tyy+2)*(Txx+2)*Tc + Tf*3*3*Tc + Txy*Tf:
        Txx=Tyy=7
        Txy=Nxy
    #L3,  Txy, Tf, Tc, -> Nf
    Input.append( (1*Tyy+2)*(1*Txx+2)*Tc*  (Nxy/Txy*Nf/Nf*Nc/Tc))
