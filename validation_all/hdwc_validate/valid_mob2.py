...
def run_instance(Txy, Tf, Tc):
    #cmd = ' icpc -O3 -Ofast -march=native -fopenmp -I../Vary_Layout_UKR/build -DLKF=16 -DLC=1 -DLOF=1 -DuNf=128 -DuNx=14 -DuNy=14 -DuNc=256 -DuNw=3 -DuNh=3 -DEdgeXY=4 -DuSx=2 -DuSy=2 testbed.cpp '
    cmd = 'icpc -O3 -Ofast -march=native -fopenmp -I../Vary_Layout_UKR/build -DLKF=16 -DLC=1 -DLOF=1 -DuNf=64 -DuNx=56 -DuNy=56 -DuNc=64 -DuNw=3 -DuNh=3 -DEdgeXY=4 -DuSx=2 -DuSy=2 testbed_likwid.cpp -DLIKWID_PERFMON -Wl,-rpath=/opt/local/lib -llikwid  '
    cmd += ' -DTxy3=' + str(Txy) 
    cmd += ' -DTf2=' + str(Tf)
    cmd += ' -DTc1=' + str(Tc)
...
def gen_pred(Nxy, Nf, Nc, Txy, Tf, Tc, cache_size, bandwidth):
    ...
    if (2*Tyy+2)*(2*Txx+2)*Tc + Tf*3*3*Tc + Txy*Tf < 3*1024*1024:
        Txy = Nxy
        Tyy = Nxy
    #L3,  Txy, Tf, Tc, -> Nf
