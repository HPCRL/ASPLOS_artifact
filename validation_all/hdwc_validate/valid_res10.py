...
#    cmd = 'icpc -O3 -Ofast -march=native -fopenmp -I../Vary_Layout_UKR/build -DLKF=16 -DLC=1 -DLOF=1 -DuNf=512 -DuNx=7 -DuNy=7 -DuNc=512 -DuNw=3 -DuNh=3 -DEdgeXY=1 -DuSx=2 -DuSy=2 testbed_likwid.cpp -DLIKWID_PERFMON -Wl,-rpath=/opt/local/lib -llikwid'
    cmd = 'icpc -O3 -Ofast -march=native -fopenmp -I../Vary_Layout_UKR/build -DLKF=16 -DLC=1 -DLOF=1 -DuNf=512 -DuNx=7 -DuNy=7 -DuNc=256 -DuNw=3 -DuNh=3 -DEdgeXY=1 -DuSx=2 -DuSy=2 testbed_likwid.cpp -DLIKWID_PERFMON -Wl,-rpath=/opt/local/lib -llikwid '
    cmd += ' -DTxy3=' + str(Txy) 
    cmd += ' -DTf2=' + str(Tf)
    cmd += ' -DTc1=' + str(Tc)
...
def gen_pred(Nxy, Nf, Nc, Txy, Tf, Tc, cache_size, bandwidth):
    ...
    # reg,  Rxy, Rf, 1 -> Tc
    Input.append( (2*1+2)*(2*Rxy)*3*Nc  *  R_tiles )
    Kernel.append( Rf*3*3*Nc * R_tiles)
    Output.append( Rxy*Rf * Nc/Tc * R_tiles)
    if Tf == Rf:
        Rxy=Txy
    #L1,  Rxy, Rf, Tc, -> Tf
    Input.append( (2*1+2)*(2*Rxy+2)*Tc *  (Nxy/Rxy*Nf/Tf*Nc/Tc)   )
    Kernel.append( Tf*3*3*Tc * (Nxy/Rxy*Nf/Tf*Nc/Tc)  )
    Output.append( Rxy*Tf * (Nxy/Rxy*Nf/Tf*Nc/Tc)   )

    #L2,  Rxy, Tf, Tc, -> Txy

    Txx = 7 if Txy>=7 else Txy
    Tyy = 1 if Txy<=7 else Txy/7
    #Tyy = Txy
    #Txx = 1

    Input.append(  (2*Tyy+2)*(2*Txx+2)*Tc  *  (Nxy/Txy*Nf/Tf*Nc/Tc) )
    Kernel.append( Tf*3*3*Tc  * (Nxy/Txy*Nf/Tf*Nc/Tc))
    Output.append( Txy*Tf  * (Nxy/Txy*Nf/Tf*Nc/Tc))
    if (2*Tyy+2)*(2*Txx+2)*Tc + Tf*3*3*Tc + Txy*Tf:
        Txx=Tyy=7
        Txy=Nxy
    #L3,  Txy, Tf, Tc, -> Nf
    Input.append( (2*Tyy+2)*(2*Txx+2)*Tc*  (Nxy/Txy*Nf/Nf*Nc/Tc))
