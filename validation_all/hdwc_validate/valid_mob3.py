...
def run_instance(Txy, Tf, Tc):
    ...
    cmd ='icpc -O3 -Ofast -march=native -fopenmp -I../Vary_Layout_UKR/build -DLKF=16 -DLC=1 -DLOF=1 -DuNf=128 -DuNx=56 -DuNy=56 -DuNc=128 -DuNw=3 -DuNh=3 -DEdgeXY=4 -DuSx=1 -DuSy=1 testbed_likwid.cpp -DLIKWID_PERFMON -Wl,-rpath=/opt/local/lib -llikwid '
    cmd += ' -DTxy3=' + str(Txy) 
    cmd += ' -DTf2=' + str(Tf)
    cmd += ' -DTc1=' + str(Tc)
...
def gen_pred(Nxy, Nf, Nc, Txy, Tf, Tc, cache_size, bandwidth):
    ...
    if (1+2)*(Txy+2)*Tc + Tf*3*3*Tc + Txy*Tf < 3*1024*1024:
        Txy = Nxy        
    #L3,  Txy, Tf, Tc, -> Nf
