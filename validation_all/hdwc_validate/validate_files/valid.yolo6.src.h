...
#include "omp.h"
#include "transpose.h"
#include "gen_ukr_A6B2gemm_1_128_68_68_256_1_1.h"
#include "gen_ukr_A4B2gemm_1_128_68_68_256_1_1.h"
...
{
for(int xy3=xy4;xy3<min(4624, 4624+xy4);xy3+=Txy3)
{
for(int f3=f4;f3<min(128, 128+f4);f3+=Tf2)
{
for(int c3=c4;c3<min(256, 256+c4);c3+=Tc1)
{
for(int xy2=xy3;xy2<min(4624, Txy3+xy3);xy2+=6)
{
...
int offsetA=0+b1*1183744+c1_1*4624+1*x1*68+1*y1*1+c1_2*1;
int offsetB=0+kf1_1*4096+c1*16+0*16+0*16+kf1_2*1;
int offsetC=0+b1*591872+of1_1*4624+x1*68+y1*1+of1_2*1;
