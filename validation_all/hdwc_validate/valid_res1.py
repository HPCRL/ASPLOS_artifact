...
def run_instance(Txy, Tf, Tc):
    cmd = 'icpc -O3 -Ofast -march=native -fopenmp -I../Vary_Layout_UKR/build -DLKF=16 -DLC=1 -DLOF=1 -DuNf=64 -DuNx=112 -DuNy=112 -DuNc=3 -DuNw=7 -DuNh=7 -DEdgeXY=4 -DuSx=2 -DuSy=2 testbed_likwid.cpp -DLIKWID_PERFMON -Wl,-rpath=/opt/local/lib -llikwid '
    cmd += ' -DTxy3=' + str(Txy) 
    cmd += ' -DTf2=' + str(Tf)
    cmd += ' -DTc1=' + str(Tc)
...
def gen_pred(Nxy, Nf, Nc, Txy, Tf, Tc, cache_size, bandwidth):
    ...
    # reg,  Rxy, Rf, 1 -> Tc
    Input.append( (2*1+6)*(2*Rxy)*7*Nc  *  R_tiles )
    Kernel.append( Rf*7*7*Nc * R_tiles)
    Output.append( Rxy*Rf * Nc/Tc * R_tiles)
    if Tf == Rf:
        Rxy=Txy
    #L1,  Rxy, Rf, Tc, -> Tf
    Input.append( (2*1+6)*(2*Rxy+6)*Tc *  (Nxy/Rxy*Nf/Tf*Nc/Tc)   )
    Kernel.append( Tf*7*7*Tc * (Nxy/Rxy*Nf/Tf*Nc/Tc)  )
    ...
    Input.append(  (2*Tyy+6)*(2*Txx+6)*Tc  *  (Nxy/Txy*Nf/Tf*Nc/Tc) )
    Kernel.append( Tf*7*7*Tc  * (Nxy/Txy*Nf/Tf*Nc/Tc))
    Output.append( Txy*Tf  * (Nxy/Txy*Nf/Tf*Nc/Tc))

    if (2*Tyy+6)*(2*Txx+6)*Tc + Tf*7*7*Tc + Tf*Txy < 3*1024*1024:
        Txy=Nxy
    #L3,  Txy, Tf, Tc, -> Nf
    Input.append( (2*Tyy+6)*(2*Txx+6)*Tc*  (Nxy/Txy*Nf/Nf*Nc/Tc))
    Kernel.append( Nf*7*7*Tc*  (Nxy/Txy*Nf/Nf*Nc/Tc))
