...
def autogen_run():
    ...
    modgen = ModGen(glb_range=global_range, tensors=[tsA, tsB, tsC],
                    ...
                    capacity_list=[128, 32*1024/4, 1024*1024/4, 24*1024*1024/4],
                    level_list=[0,1,2,4],
                    bw_list=[65, 57, 28, 16],
...
def autogen_run_avx5(Nb, Nf, Nx, Ny, Nc, Nw, Nh,  kf, of, ic, parallel, numAB=[6,16], strideXY=[1,1]):
    print (' auto gen run')
    global_range = {'b':Nb, 'f':Nf, 'x':Nx, 'y':Ny, 'c':Nc, 'w':Nw, 'h':Nh}
    accessA = [{'b':1}, {'c':1}, {'x':1, 'w':1}, {'y':1, 'h':1}]
    accessB = [{'f':1}, {'c':1}, {'w':1}, {'h':1}]
    ...
    modgen = ModGen(glb_range=global_range, tensors=[tsA, tsB, tsC],
                    ...
                    fp_coeffs={'A': 1, 'B': 1, 'C': 1},
                    cost_coeffs={'A': 1, 'B': 1, 'C': 2},
                    capacity_list=[512, 32*1024/4, 256*1024/4, 12*1024*1024/4],
                    level_list=[0,1,2,4],
                    bw_list=[65, 57, 28, 16],
                    parallel_list=[3], parallelism = [parallel], parallel_ids=['x', 'y' ,'f'], numAB=numAB)
    modgen.build_all_lv_cost_fp()
#    solution = modgen.create_nest_cost_map()
    ...
    for solution in solution_list:
        ...
        # runner.run_one_from_batch()                                                                  #
        ################################################################################################
        runner = Avx5runner(pbsz=[Nb, Nf,Nx,Ny,Nc,Nw,Nh],
                        split_fc=[kf,ic,of],
                       config=gen_config, parallel_fork=select_fork, numAB=numAB, strideXY=strideXY)
        runner.run_one_from_batch()
...
def main():
    ...
    autogen_run_avx5(Nb=Nb, Nf=Nf, Nc=Nc, Nx=Nx, Ny=Ny, Nw=Nw, Nh=Nh, kf=kf, of=of, ic=ic, parallel=par, numAB=[numA, numB], strideXY=[strdX, strdY])
#    autogen_run()
    #sc_batched_run()
    #    single_run()
