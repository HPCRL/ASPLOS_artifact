...
@autotvm.task.register("topi_x86_conv2d_NCHW_test")
def _topi_nn_conv2d_NCHWc(*args, **kwargs):
    ...
    if len(args) == 7:
        ...
        data, kernel, strides, padding, dilation, origin_layout, dtype, config = args
    else:
        assert len(args) == 8
    ...
    C = _declaration_conv(cfg, data, kernel, strides, padding, dilation,
                          origin_layout, dtype)
    s = schedule_conv2d(cfg, [C])
