...
## Trouble shooting
* We noticed that there are some compatibility issue when ICC2019 and GCC9 are installed together.  Detailed information can be found at Intel's forum:
  ...
  To fix this problem we have added flag "--std=c++11" and passed testing on bantu 20.04 and gcc9.3.  If users are still encoutering this issue, please consider donwgrade gcc version to gcc7.5.
* An assertion error "assert(len(copy_candidate_list)>0)" is suggesting that the software cannot find any available solution.  In most of cases we have ever seen, it is because the software cannot find/launch the solver.  If some unusual problem size or architecture out of the scope we have tested triggered this issue, please read section Extensions and Variance
* ipopt.opt is the solver configuration file, by default we provide this file with repository. missing/modification of this file will results changes of solver behavior.  There is not yet a clear conclusion of how could it change the final perfromance. 
...
* To maintain the stability of performance, it is required to guarantee there are no background process/deamon alive when runing our experiments.  This is more likely to happen on reserarch/public clusters, where user has no permission to shutdown deamon processes.  Failure of clean up the background will result very unstable performance measurement.
* To maintain the stability of performance, it is required to disable hyperthreads, or any techniques that can map more than 1 thread on the same phsical core.
* To maintain the stability of performance, it is suggested to disable DVFS, or any techniques that can dynamically change the frequency.
...
* Running on a different CPU with different cache size might require changing the predefined cache capacity in main.py (variable 'capacity_list')
* Running on a CPU equiped with other ISA could require new micro-kernel design/development.
* In case of the solver cannot find a solution, but the solver itself is configurated correctly, it could because there are too many cores used for a small benchmark, please consider to reduce the cores or increase the problem size.  
