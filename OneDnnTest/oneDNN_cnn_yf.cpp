...
void test_conv(engine::kind engine_kind, long long nb, long long nw, long long ns, long long nx, long long ny, long long nf, long long nc, long long sw, long long sh){
...
//        const int B = 1, W = 3, S = 3, X = 224, Y = 224, F = 64, C =3;
    ...
    const long long stride_ib = C*(X+W-1)*(Y+S-1);
    const long long stride_ic = (X+W-1)*(Y+S-1);
    const long long stride_ix = (Y+S-1);
...
//                                  output_md, conv_strides, conv_padding, conv_padding);
        ...
        if(0 && conv_prim_desc.weights_desc() != user_kernel_mem.get_desc()){        
...
int main(int argc, char**argv){
	...
	std::cout<<"hello\n";
	test_conv(engine_kind, nb, nw, nh, nx, ny, nf, nc, sw, sh);
        std::cout<<" conv pass\n";        
    }catch(dnnl::error &e){
